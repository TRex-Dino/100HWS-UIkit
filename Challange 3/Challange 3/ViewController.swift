...
//  Challange 3
//
//  Created by Dmitry on 03.07.2021.
//
...
class ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        displayedLattersLabel.font = UIFont.systemFont(ofSize: 25)
        ...
        if hiddenWord.contains(Character(tappedLetter)) {
            ...
            for (index, hiddenLetter) in hiddenWord.enumerated() {
                if String(hiddenLetter) == tappedLetter {
                    var chars = Array(displayedLatters)
                    chars[index] = Character(tappedLetter)
                    displayedLatters = String(chars)
                }
            }
        } else {
            self.attempsLeft -= 1
